...
// 		return []TreeNode{{Name: capitalize(target)}}, counter
// 	}
// 	if elem, ok := elementMap[target]; !ok || len(elem.Recipes) == 0 {
// 		return []TreeNode{}, counter
// 	}
// 	trees := bfsBuildRecipeTrees(target, elementMap, maxRecipes, &counter)
// 	fmt.Println("Total node visited:", counter) // Atau simpan ke return kalau ingin digunakan
// 	return trees, counter
// }

// func bfsBuildRecipeTrees(target string, elementMap map[string]Element, maxRecipes int, counter *int) []TreeNode {
